...
if cgi["select_id_only"] != "" then
  ...
  onlyDB = "SELECT kojin.id, kojin.name,all_test.testname,all_test.testday, res_test.jpn, res_test.math, res_test.eng, res_test.sci
...
print <<EOM
  ...
  <input type="submit" value="検索" onClick="hoji()">
  </p>
  </form>
...
<tr>
<th>テスト名</th><th>テスト実施日</th><th>国語</th><th>数学</th><th>英語</th><th>理科</th>
</tr>
EOM
...
for k in 0..k -1
  puts ("<tr>")
  puts ("<td>#{only_testname[k]}</td><td>#{only_day[k]}</td><td>#{only_jpn[k]}</td><td>#{only_math[k]}</td><td>#{only_eng[k]}</td><td>#{only_sci[k]}</td>")
  puts ("</tr>")
end
